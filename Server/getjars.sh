...
mkdir -p libs; 
(cd libs;
    curl -s http://ftp.heanet.ie/mirrors/www.apache.org/dist/httpcomponents/httpcore/binary/httpcomponents-core-4.3.3-bin.tar.gz | tar -xzf - ;
    curl -s http://ftp.heanet.ie/mirrors/www.apache.org/dist//httpcomponents/httpclient/binary/httpcomponents-client-4.3.4-bin.tar.gz | tar -xzf -;
    curl -s http://ftp.heanet.ie/mirrors/www.apache.org/dist/commons/io/binaries/commons-io-2.4-bin.tar.gz | tar -xzf -;
    curl -s http://ftp.heanet.ie/mirrors/www.apache.org/dist/commons/lang/binaries/commons-lang3-3.4-bin.tar.gz | tar -xzf -;
    ...
    find . -name httpclient-4.3.4.jar -print | (while read j; do cp -f $j .; done)
    find . -name httpcore-4.3.3.jar -print | (while read j; do cp -f $j .; done)
    ...
    find . -name commons-lang3-3.4.jar | (while read j; do cp -f $j .; done)
